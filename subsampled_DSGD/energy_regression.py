...
if __name__ == "__main__":
    ...
    NN_ARCHITECTURE = [
        {"input_dim": data_train.shape[1]-1, "output_dim": 80, "activation": "relu"},
        {"input_dim": 80, "output_dim": 40, "activation": "relu"},
        {"input_dim": 40, "output_dim": 1, "activation": "linear"},
    ]
    ...
    rmse = np.sqrt(mse)*target_offsets[1]
    if args.save_data:
        np.save('simulation_data/energy_loss_'+args.gtype+'_'+str(args.subsample_ratio)+'.npy', ave_loss)
        np.save('simulation_data/energy_mse_'+args.gtype+'_'+str(args.subsample_ratio)+'.npy', rmse)
